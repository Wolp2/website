...
export async function onRequestGet({ env, request }) {
  ...
  if (!expected || expected !== state) return new Response("Bad state", { status: 400 });
  ...
  // tokenJson: access_token, refresh_token, expires_in, scope, user_id, etc.
  await env.FITBIT_KV.put("fitbit_tokens", JSON.stringify({
    ...tokenJson,
    obtained_at: Date.now()
  }));

  // send you back to your dashboard
